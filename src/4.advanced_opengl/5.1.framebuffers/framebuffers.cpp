...
int main()
{
	...
	if (bUseDSA)
	{
		glProgramUniform1i(shader.ID, glGetUniformLocation(shader.ID, "texture1"), 0);
	}
	else
	{
		shader.use();
	...
	if (bUseDSA)
	{
		glProgramUniform1i(shader.ID, glGetUniformLocation(shader.ID, "screenTexture"), 0);
	}
	else
	{
		screenShader.use();
    ...
    if (bUseDSA)
    {
        ...
        //glTextureSubImage2D(textureColorbuffer, 0, 0,0,SCR_WIDTH, SCR_HEIGHT, GL_RGB, GL_UNSIGNED_BYTE, NULL); // 上传纹理图像数据
